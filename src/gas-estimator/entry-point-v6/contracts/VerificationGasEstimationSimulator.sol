...
contract EntryPoint is IEntryPoint, StakeManager, NonceManager, ReentrancyGuard {
    ...
    function simulateHandleOp(UserOperation calldata op, address target, bytes calldata targetCallData) public override 
        returns (
            uint256 preOpGas, 
            uint256 paid, 
            uint48 validAfter,
        ...
        )
    {
        ...
        return (opInfo.preOpGas, paid, data.validAfter, data.validUntil, targetSuccess, targetResult);
    }
...
error DebugError(bytes reason);

contract VerificationGasEstimationSimulator is EntryPoint {
    using Math for uint256;

    struct EstimateVerificationGasArgs {
        UserOperation op;
        uint256 minGas;
        uint256 maxGas;
        uint256 rounding;
    ...
    error EstimateVerificationGasResult(uint256 gasEstimate, uint48 validAfter, uint48 validUntil, uint256 numRounds);

    error EstimateVerificationGasContinuation(uint256 minGas, uint256 maxGas, uint48 validAfter, uint48 validUntil, uint256 numRounds);

    error EstimateVerificationGasRevertAtMax(bytes revertData);

    error SimulateOnlyValidationOOG(bytes oogMsg);

    error FailedOpError(bytes reason);

    error SimulateOnlyValidationRevert(uint256 simulateOnlyValidationGasUsed, uint48 validAfter, uint48 validUntil);

    /**
     * Runs a binary search to find the smallest amount of gas at which the verification step
     * succeeds.
     *
     ...
     * initial upper bound.
     */
    function estimateVerificationGas(EstimateVerificationGasArgs calldata args) external {
        ...
        uint256 numRounds = 0;
        // make one call at max gas to make sure success is even possible
        try this.simulateOnlyValidation(args.op, args.maxGas) {} catch (bytes memory simulateOnlyValidationRevertData) {
            if(bytes4(simulateOnlyValidationRevertData) == IEntryPoint.FailedOp.selector) {
                revert FailedOpError(simulateOnlyValidationRevertData);
            }
            assembly {
                simulateOnlyValidationRevertData := add(simulateOnlyValidationRevertData, 0x04)
            }
            (uint256 simulateOnlyValidationGasUsed, uint48 validAfter, uint48 validUntil) = abi.decode(
                simulateOnlyValidationRevertData,
                (uint256, uint48, uint48)
            );
            uint256 scaledGuess = (simulateOnlyValidationGasUsed * 2) / args.rounding;
            while (scaledMaxFailureGas + 1 < scaledMinSuccessGas) {
                ...
                if (!isEnoughGasForGuess(guess)) {
                    ...
                    revert EstimateVerificationGasContinuation(nextMin, nextMax, validAfter, validUntil, numRounds);
                }
                (uint256 gasUsed, , , bool success) = innerCall(
                    args.op,
                    guess
                );
                if (success) {
            ...
            revert EstimateVerificationGasResult(
    ...
    function simulateOnlyValidation(UserOperation calldata op, uint256 gas) public {
        ...
        uint256 simulateOnlyValidationGasUsed = initialGas - gasleft();
        if(simulateOnlyValidationGasUsed > gas) {
            revert SimulateOnlyValidationOOG("OOG");
        }
        revert SimulateOnlyValidationRevert(
            simulateOnlyValidationGasUsed,
            data.validAfter,
            data.validUntil
        );
    }
    ...
    function innerCall(
        ...
        uint256 gas
    ) private returns (uint256 gasUsed, uint48 validAfter, uint48 validUntil, bool success) {
        try this.simulateOnlyValidation(op, gas) {
            // Should never happen. _innerCall should always revert.
            revert("Revert outside simulateOnlyValidation");
        } catch (bytes memory simulateOnlyValidationRevertData) { // handle with SimulateOnlyValidationOOG or FailedOp Error
            if(bytes4(simulateOnlyValidationRevertData) == SimulateOnlyValidationOOG.selector) {
                return (0, 0, 0, false);
            }
            ...
            success = true;
           (gasUsed, validAfter, validUntil) = abi.decode(
                simulateOnlyValidationRevertData,
                (uint256, uint48, uint48)
            );
        }
