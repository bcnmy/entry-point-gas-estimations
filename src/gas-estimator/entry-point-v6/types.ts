...
export const signatureValidationFailedSchema = z.tuple([addressSchema]).transform((val) => ({ aggregator: val[0] }));
...
export const failedOpSchema = z.tuple([z.bigint(), z.string()]).transform((val) => ({ opIndex: val[0], reason: val[1] }));
    ...
    cause: z.object({
        ...
        cause: z.object({
            data: z.string().transform((val) => {
                ...
                const errorResult = decodeErrorResult({ abi: ENTRY_POINT_ABI, data: errorHexData });
