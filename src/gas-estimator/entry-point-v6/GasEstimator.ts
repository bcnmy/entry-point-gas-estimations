...
import { DEFAULT_ENTRY_POINT_ADDRESS, DEFAULT_EXECUTE_SIMULATOR_BYTE_CODE, } from './constants';
...
export class GasEstimator {
  ...
  async simulateHandleOp(params: SimulateHandleOpParamsType): Promise<SimulateHandleOpReturnType> {
    ...
    try {
      ...
      const decodedError = decodeErrorResult({ abi: ENTRY_POINT_ABI, data: cause.data });
  ...
  async esitmateUserOperationGasWithNoEthCallStateOverrideSupport(
    ...
    const executionResult = this.getSimulationResult(errorResult, "execution") as ExecutionResult;    

    const verificationGasLimit = ((executionResult.preOpGas - userOperation.preVerificationGas) * 6n) / 5n;
